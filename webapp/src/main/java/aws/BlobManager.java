...
public class BlobManager {
    ...
    public String createMultiPart(int fileId) {
        CreateMultipartUploadRequest multipartReq = CreateMultipartUploadRequest.builder().bucket(bucket)
                .key(getFileDataFilePath(fileId)).build();
    ...
    public String uploadPart(int fileId, String uploadId, int partNumber, InputStream in, long len) {
        UploadPartRequest req = UploadPartRequest.builder().bucket(bucket).key(getFileDataFilePath(fileId))
                .uploadId(uploadId).partNumber(partNumber + 1) // [MEMO] partNumber starts from 1
                .build();
    ...
    public void completeMultiPart(int fileId, String uploadId, List<String> etags) {
        ...
        for (int i = 0; i < etags.size(); i++) {
            ...
            CompletedPart part = CompletedPart.builder().partNumber(i + 1) // [MEMO] partNumber starts from 1
                    .eTag(etag).build();
            parts.add(part);
        }
        ...
        CompleteMultipartUploadRequest req = CompleteMultipartUploadRequest.builder().bucket(bucket)
                .key(getFileDataFilePath(fileId)).uploadId(uploadId).multipartUpload(completedMultipartUpload).build();
        s3Client.completeMultipartUpload(req);
    }

    public InputStream getFileDataInputStream(int fileId) {
        GetObjectRequest req = GetObjectRequest.builder().bucket(bucket).key(getFileDataFilePath(fileId)).build();
        return s3Client.getObject(req);
    }

    public long getFileSize(int fileId) {
        HeadObjectRequest req = HeadObjectRequest.builder().bucket(bucket).key(getFileDataFilePath(fileId)).build();
        HeadObjectResponse res = s3Client.headObject(req);
        return res.contentLength();
    }

    public void deleteAll() {
        ListObjectsRequest listReq = ListObjectsRequest.builder().bucket(bucket).prefix(sessionKey + "/").build();
        ListObjectsResponse listRes = s3Client.listObjects(listReq);
        for (S3Object obj : listRes.contents()) {
            DeleteObjectRequest req = DeleteObjectRequest.builder().bucket(bucket).key(obj.key()).build();
    ...
    public InputStream getZipFileDataInputStream() {
        GetObjectRequest req = GetObjectRequest.builder().bucket(bucket).key(getZipFileDataFilePath()).build();
        return s3Client.getObject(req);
    }

    public long getZipFileSize() {
        HeadObjectRequest req = HeadObjectRequest.builder().bucket(bucket).key(getZipFileDataFilePath()).build();
        HeadObjectResponse res = s3Client.headObject(req);
        return res.contentLength();
    }
    ...
    private static class S3OutputStream extends OutputStream {
        ...
        public S3OutputStream(S3Client s3Client, String bucket, String key) {
            ...
            CreateMultipartUploadRequest multipartReq = CreateMultipartUploadRequest.builder().bucket(bucket).key(key)
                    .build();
            CreateMultipartUploadResponse multipartRes = s3Client.createMultipartUpload(multipartReq);
        ...
        public void flush() {
            ...
            UploadPartRequest req = UploadPartRequest.builder().bucket(bucket).key(key).uploadId(uploadId)
                    .partNumber(partNumber).build();
            UploadPartResponse res = s3Client.uploadPart(req, RequestBody.fromBytes(buff.toByteArray()));
            buff.reset();

            CompletedPart part = CompletedPart.builder().partNumber(partNumber).eTag(res.eTag()).build();
            parts.add(part);
        }
